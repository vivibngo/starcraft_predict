...
# %% [markdown]
# If the ActionLatency is <= 98.2, the tree will follow the left branch from this node. The Gini impurity score = 0.811 suggests that the samples in this node are distributed across multiple LeagueIndex rather than being predominantly in a single LeagueIndex. Samples = 1662 represents the number of samples(data points) that reached this node during the training processes. The value = [161, 281, 439, 652, 630, 476, 31] shows the distribution of target LeagueIndex, the values correspond to the counts of each LeagueIndex Rank, for example there are 161 samples with LeagueIndex 1, 281 with LeagueIndex 2, and so on.
